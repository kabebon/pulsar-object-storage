...
.PHONY: help tidy build run dev test fmt vet generate templates migrate-up migrate-down docker-up docker-down docker-logs docker-build psql redis-cli minio-mc
...
docker-build: ## Build the app image only
	docker compose -f deploy/docker-compose.yml build pulsar

# ----------------- Inspect -----------------

psql: ## Connect to Postgres via psql
	@docker compose -f deploy/docker-compose.yml exec postgres psql -U pulsar -d pulsar
